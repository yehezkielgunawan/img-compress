...
/**
 * Decode an image blob into a drawable source. Two-step fallback:
 *   1. createImageBitmap(data) — no options (fast, GPU-backed)
 *   2. <img> element via Object URL — universally supported fallback
 *
 * No resize options are passed to createImageBitmap. Mobile Safari doesn't
 * support them, and mobile Chrome has stale-handle / HEIF-as-JPEG issues
 * with options. Original dimensions come from the decoded result itself.
 */
const decodeImage = async (
	...
	// Step 1: createImageBitmap with NO options
	try {
	...
	// Step 2: <img> element via Object URL
...
/**
 * Extract RGB pixel data from a Blob using decodeImage + canvas.
 * Progressive retry: tries 4096 → 2048 → 1024 max dimension so mobile
 * devices with tight memory budgets can still succeed at smaller resolution.
 *
 * Dimensions come from the decoded bitmap/img — no header parsing needed.
 */
	...
	try {
		for (const maxDim of DECODE_MAX_DIMENSIONS) {
			...
			try {
				...
				if (
					maxDim === DECODE_MAX_DIMENSIONS[DECODE_MAX_DIMENSIONS.length - 1]
				) {
					throw err;
				}
		...
		// Clean up decoded source
		if ("close" in decoded.source) {
...
export const PixoCompressor = () => {
	...
	const [decodedPixels, setDecodedPixels] = useState<DecodedPixels | null>(
		null,
	);
	...
	const handleFileSelect = async (selectedFile: File): Promise<void> => {
		...
		// Increment generation — any in-flight decode for a previous file
		// will see a stale gen and bail out.
		const gen = ++decodeGenRef.current;

		// Deterministic cleanup of previous preview URL
		...
		setResult(null);
		setDecodedPixels(null);
		setError("");
		setPreviewFailed(false);

		// Read file into memory immediately while the handle is guaranteed
		// valid. Mobile browsers invalidate File handles between async ops.
		let blob: Blob;
		try {
			const buffer = await selectedFile.arrayBuffer();
			blob = new Blob([buffer], { type: selectedFile.type });
		} catch {
		...
		// Set file metadata and preview URL
		setFile(selectedFile);
		const previewUrl = URL.createObjectURL(blob);
		previewUrlRef.current = previewUrl;
		setOriginalUrl(previewUrl);

		// Decode pixels immediately in the same handler task (Bug #8 fix).
		// NOT in useEffect — mobile browsers can invalidate handles between
		// async tasks, and useEffect fires in a new microtask.
		setIsLoading(true);
		try {
			const pixels = await extractPixels(blob);
			if (gen !== decodeGenRef.current) return;
			setDecodedPixels(pixels);
			// Don't setIsLoading(false) — the compression effect will manage it
		} catch (err) {
			if (gen !== decodeGenRef.current) return;
					...
					<div class="card bg-base-100 shadow-xl">
						<div class="card-body p-4 md:p-6">
							...
							<div class="mt-2 space-y-1 text-base-content/70 text-xs md:text-sm">
								...
								{decodedPixels && (
									<p>
										<strong>Dimensions:</strong> {decodedPixels.originalWidth} x{" "}
										{decodedPixels.originalHeight}
									</p>
								)}
								<p>
